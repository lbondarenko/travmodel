...
<a href="#" data-add="manual">✏️&nbsp; Enter picks manually</a>
</div>
</div>
...
document.addEventListener("click",function(ev){
  ...
  if(item&&m){ ev.preventDefault(); m.classList.remove("open");
    ...
    else if(k==="manual"){ var d=document.getElementById("tixmanual");
...
CSS = """
  ...
  *{ box-sizing:border-box; } html,body{ margin:0; }
  ...
  .dwrail{ position:fixed; right:0; top:42%; z-index:70; display:flex;
    flex-direction:column; gap:8px; transition:transform .25s ease; }
  .dwrail.shift{ transform:translateX(calc(-1 * var(--dww))); }
  @media (prefers-reduced-motion: reduce){ .dwrail{ transition:none; } }
  ...
  .slipd{ width:var(--dww); height:100%; overflow-y:auto;
  ...
  .epanel{ width:var(--dww); height:100%; overflow-y:auto;
...
function dwGo(id){
  ...
  var rail=document.querySelector('.dwrail');
  if(rail)rail.classList.toggle('shift', opening);
  try{localStorage.setItem('tm_dw', opening?id:'');}catch(err){}
}
try{var o=localStorage.getItem('tm_dw'); if(o){var el=document.getElementById(o); if(el){el.classList.add('open'); var r=document.querySelector('.dwrail'); if(r)r.classList.add('shift');}}}catch(err){}
</script>""")
...
<ul class='infoul'><li>{b1}</li><li>{b2}</li></ul></div></article>""")
  ...
  <div class="dstamp">{hits} AV {nlegs}<small>L\u00c5ST F\u00d6RE START \u00b7 AUTOR\u00c4TTAD</small></div>"""
    ...
    for tier, info in (pool_payouts or {}).items():
        ...
        if rows_t and pay:
            win_kr += rows_t * pay
            win_parts.append(f"{rows_t} row(s) with {tier} correct at {pay:,.0f} kr".replace(",", " "))
        elif rows_t and isinstance(info, dict) and info.get("jackpot"):
            unpaid.append(f"{rows_t} row(s) with {tier} correct \u2014 below the minimum payout, rolled into the jackpot")
    net = win_kr - ticket["cost"]
    if win_kr > 0:
        money = (f"Winnings: {' + '.join(win_parts)} = <b>{win_kr:,.0f} kr</b>".replace(",", " ")
                 + f" \u00b7 stake {ticket['cost']:.0f} kr \u00b7 net <b>{net:+,.0f} kr</b>".replace(",", " "))
    elif pool_payouts and unpaid:
        money = f"{'; '.join(unpaid)} \u00b7 net \u2212{ticket['cost']:.0f} kr."
    elif pool_payouts:
